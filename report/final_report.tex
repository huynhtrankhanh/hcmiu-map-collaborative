...
From a database perspective, the most important contribution is an explicit document-and-edge design in which content nodes are stored in document collections and semantic links are persisted in edge collections, enabling relationship-centric queries (reference discovery, full-text exploration, shortest connection paths between entities, and notification fan-out). Beyond implementation details, this version of the report adds a stronger research grounding: it compares multi-model design to polyglot persistence trade-offs, discusses consistency and real-time delivery semantics, and examines graph query workloads in terms of complexity and maintainability. The report documents architecture, schema decisions, indexing and data generation strategy, representative algebraic and query operations, implementation outcomes, and limitations. The system demonstrates that a multi-model DBMS can reduce impedance mismatch in connected applications compared with separating document storage and graph traversal into different engines, while still requiring careful operational design around access control, indexing, and event ordering.
...
Campus-scale indoor information systems have two data characteristics that are often in tension: high schema variability (new kinds of room notes, comments, social interactions, moderation records) and high relationship density (references, follows, replies, dispute participants, and notification audiences). Traditional designs that split these concerns across separate systems can increase operational overhead and query complexity, an issue aligned with the broader argument that one-size-fits-all data management is rarely optimal for modern applications \cite{stonebraker2005onesize}.

For this project, the motivation was to evaluate whether a single multi-model engine could support both rapid feature evolution and relationship-heavy analytics without introducing external graph middleware. The platform therefore serves as both a practical campus tool and an applied DBMS experiment focused on model fit, developer productivity, and query expressiveness.
...
A second rationale is architectural simplicity under continuous iteration. During development, feature additions (trial negotiation history, map-linked comments, notifications) altered both entity structure and relationship depth. A multi-model database reduced migration friction by allowing document enrichment without sacrificing graph traversals. This design choice follows broader system research that warns against forcing all workloads into a single rigid model while still emphasizing careful workload-oriented tuning \cite{stonebraker2005onesize}.

\section{Research-grounded design considerations}
\subsection{Connected-data modeling}
The platform adopts a property-graph mindset for interaction semantics: references and follows are first-class edges rather than embedded arrays only. This enables path-based reasoning (e.g., degree of separation) with explicit traversal semantics, which is difficult to maintain in purely denormalized document structures as relationship depth grows \cite{robinson2015graph}.

\subsection{Consistency and real-time behavior}
The collaboration experience uses a pragmatic consistency model: REST endpoints persist canonical state, then WebSocket events notify clients to refresh or patch views. This approach prioritizes responsiveness while accepting short-lived read-after-write lag on some clients, a trade-off consistent with eventual-consistency discussions in distributed systems \cite{vogels2009eventually}. In practice, deterministic IDs and server-side timestamps reduce ambiguity when multiple users interact concurrently.

\subsection{Algorithmic foundations for navigation}
Indoor routing components rely on weighted graph search for shortest paths and combinational route planning for multi-stop visits. Dijkstra's algorithm provides deterministic optimality under non-negative edge costs, while dynamic-programming TSP formulations motivate the bounded multi-stop planner used in the UI \cite{dijkstra1959,bellman1962}. These choices balance correctness and computational feasibility for campus-scale graphs.
...
\begin{itemize}
    \item \textbf{Entity listing/filtering:} index candidates include \texttt{type}, \texttt{parentEntityId}, and \texttt{createdAt}.
    \item \textbf{Notification retrieval:} index candidates include \texttt{userId} and \texttt{read} for inbox-like queries.
    \item \textbf{Edge traversals:} edge collections already optimize \texttt{\_from}/\texttt{\_to}; traversal cost is primarily controlled by graph diameter and branch factor.
    \item \textbf{Map lookup:} deterministic map-entity IDs reduce dependence on expensive fuzzy matching at request time.
\end{itemize}
...
\begin{itemize}
    ...
    \item Research endpoints expose relationship analytics that are reusable by external scripts and future dashboards.
\end{itemize}
...
\begin{itemize}
    ...
    \item A generic entity model accelerated feature delivery, but required disciplined type checks to avoid semantic drift.
\end{itemize}

\textbf{Evaluation notes.}
Validation was performed through automated API and browser scenarios included in the repository. The API suite focuses on authentication, collaborative entity lifecycle, follow/notification behavior, and trial transitions. End-to-end scenarios exercise map lookup and cross-page workflows under a production-like compose deployment. While these tests are functional (not load tests), they provide confidence that major workflow invariants hold after iterative changes.

\textbf{Algorithmic component.}
Shortest-route and multi-stop planning are consistent with classic graph optimization foundations (Dijkstra for shortest path, dynamic programming lineage for TSP variants) \cite{dijkstra1959,bellman1962}. For user experience, path quality is constrained less by algorithmic correctness and more by graph modeling fidelity (missing connectors, inaccurate floor transitions, or stale room metadata).
...
\begin{itemize}
    \item \textbf{External validity:} the dataset is campus-specific; behavior on larger institutions with denser interaction graphs may differ.
    \item \textbf{Performance validity:} no formal throughput/latency benchmark was executed, so scalability conclusions remain qualitative.
    \item \textbf{Behavioral validity:} synthetic and developer-driven interactions may not reflect long-term real user moderation dynamics.
\end{itemize}
...
\section{Future work}
\begin{itemize}
    \item Add ArangoSearch views and ranking for richer retrieval quality, including BM25-based relevance tuning for entity search.
    \item Introduce fine-grained authorization roles and moderation policies (e.g., trusted editors, temporary suspensions, and audit trails).
    \item Evaluate query performance under larger synthetic workloads with repeatable benchmark scripts and dashboarded latency percentiles.
    \item Integrate optional indoor positioning feeds and accessibility-aware routing with constraints for elevators, ramps, and restricted zones.
    \item Formalize conflict-resolution analytics from trial data (time-to-resolution, participation balance, appeal patterns) to improve governance design.
\end{itemize}
